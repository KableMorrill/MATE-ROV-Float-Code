...
// ============== HARD ENCODER LIMITS (SYRINGE PHYSICAL RANGE) ==============
// These are ABSOLUTE limits — the motor will NOT drive past these regardless
// of what commands it. Protects against syringe damage from any code path.
// Full syringe stroke is 11 motor revolutions (measured).
#define ENCODER_MIN_COUNT (PULSES_PER_REV * -11) // Max extension (descent) = -28600
#define ENCODER_MAX_COUNT 0                        // Max retraction (ascent) — home/default position
#define ENCODER_HOME_DEADBAND 50                   // ±pulses considered "at home" for return
...
void moveFloatDown(int speed) {
    if (buoyEnc.getCount() <= ENCODER_MIN_COUNT) {
        stopFloat();
        Serial.println("SAFETY: Encoder at min limit, blocking descent");
        return;
    }
    ledcWrite(IN1, 0);
    ledcWrite(IN2, speed);
...
void moveFloatUp(int speed) {
    if (buoyEnc.getCount() >= ENCODER_MAX_COUNT) {
        stopFloat();
        Serial.println("SAFETY: Encoder at max limit, blocking ascent");
        return;
    }
    ledcWrite(IN1, speed);
    ledcWrite(IN2, 0);
...
void OnDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *incomingData, int len) {
    ...
    // Manual motor control (potentiometer) - uses full speed
        ...
        Serial.println("Returning buoyancy engine to home position...");
...
void loop() {
    ...
    // Dry test sensor simulation
    // Uses profileStartTime (set when START command is received) so the
    // simulated depth ramp begins when the dive actually starts, not on boot.
    // IMPORTANT: These are SENSOR readings, not physical depths. The state
    // machine compares against sensor-offset targets (sensorTargetDeep=2.25m,
    // sensorTargetShallow=0.824m), so the simulation must match.
    float sensorDepth = 0.0;
    if (currentState >= STATE_DESCENDING && currentState <= STATE_COMPLETE) {
        unsigned long diveElapsed = currentTime - profileStartTime;
        if (diveElapsed < 30000) {
            // Descend: sensor reads 0 → sensorTargetDeep over 30s
            sensorDepth = (diveElapsed / 30000.0) * sensorTargetDeep;
        } else if (diveElapsed < 60000) {
            // Hold deep: sensor reads sensorTargetDeep for 30s
            sensorDepth = sensorTargetDeep;
        } else if (diveElapsed < 90000) {
            // Ascend: sensor reads sensorTargetDeep → sensorTargetShallow over 30s
            sensorDepth = sensorTargetDeep - (((diveElapsed - 60000) / 30000.0) * (sensorTargetDeep - sensorTargetShallow));
        } else if (diveElapsed < 120000) {
            // Hold shallow: sensor reads sensorTargetShallow for 30s
            sensorDepth = sensorTargetShallow;
        } else {
            // Remain at surface
            sensorDepth = 0.0;
        }
    ...
    switch (currentState) {
        ...
        case STATE_RETURNING_HOME: {
            ...
            if (absCount <= ENCODER_HOME_DEADBAND) {
                // Close enough to home — stop and proceed to transmit
                stopFloat();
                Serial.print("Home reached (encoder: "); Serial.print(encCount); Serial.println(")");
                currentState = STATE_TRANSMITTING;
            } else {
                // Proportional speed based on distance from home
                int speed;
                if (absCount > PULSES_PER_REV * 2) {
                    speed = MOTOR_MAX_SPEED;
                } else if (absCount > PULSES_PER_REV / 2) {
                    speed = HOLD_CORRECTION_MAX_SPEED;
                } else {
                    speed = HOLD_CORRECTION_MIN_SPEED;
                }

                if (encCount < 0) {
                    moveFloatUp(speed);  // Retract back toward 0
                } else {
                    moveFloatDown(speed);  // Extend back toward 0
                }

                // Print progress every second
                static unsigned long lastHomePrint = 0;
                if (currentTime - lastHomePrint >= 1000) {
                    lastHomePrint = currentTime;
                    Serial.print("Returning home: encoder="); Serial.print(encCount);
